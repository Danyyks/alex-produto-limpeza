...
import type { CategoryDoc } from '../../services/categories';
...
export function ProductForm({ isOpen, onClose, onSaved, categories, product }: ProductFormProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!name.trim() || !categoryId || !Number.isFinite(priceNumber) || priceNumber <= 0) {
    ...
    try {
      ...
      if (file) {
        ...
        await updateProduct(product.id, { name, description, price: priceNumber, categoryId, active });
      } else {
          ...
          >
            ...
            {categories.length === 0 ? (
              <p className="text-muted-foreground">
                Crie ao menos uma categoria antes de cadastrar um produto.
              </p>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label htmlFor="product-category" className="block mb-2 text-foreground">
                      Categoria
                    </label>
                    <select
                      id="product-category"
